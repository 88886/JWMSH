...
namespace JWMSH
{
    public partial class WorkTrackPrintCheck : Form
    {
...
left join t_User temp on ICInvBackup.FCheckerID=temp.FUserID 
...
left join t_User temp on ICInvBackup.FCheckerID=temp.FUserID where ICInvBackup.FBillNo like '%" + cOrderNumber + "%' order by FInterID desc");
        ...
        private void uGridCheck_DoubleClickCell(object sender, Infragistics.Win.UltraWinGrid.DoubleClickCellEventArgs e)
        {
            ...
            var cmd =
                new SqlCommand(
                    @"select ICInvBackup.FItemID,FShortNumber,FNumber,FModel,FName,FQty,FStockPlaceID,FBatchNo                                                                 
from ICInvBackup inner join t_ICItem on ICInvBackup.FItemID=t_ICItem.FItemID 
where FInterID=@FinterID");
        ...
        public void PrintDialog(string operation)
        {
            ...
            //判断当前打印模版路径是否存在
            var temPath = Application.StartupPath + @"\Stencil\CheckOrder.repx";

            if (!File.Exists(temPath))
            {
                MessageBox.Show(@"当前路径下的打印模版文件不存在!", @"异常", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                xtreport.Name = "CheckOrder";
