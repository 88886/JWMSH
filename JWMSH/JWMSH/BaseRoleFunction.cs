...
namespace JWMSH
{
    public partial class BaseRoleFunction : Form
    {
        ...
        {
            var btemp = false;
            foreach (DataRow dtr in baseDs.RoleFunction.Rows.Cast<DataRow>().Where(dtr => dtr.RowState != DataRowState.Deleted))
            {
                ...
                for (var i = 0; i < baseDs.RoleFunction.Rows.Count; i++)
                {
                    if (baseDs.RoleFunction.Rows[i].RowState == DataRowState.Deleted)
                        continue;
                    if (!baseDs.RoleFunction.Rows[i]["rCode"].ToString().Equals(dtr["rCode"].ToString()) ||
                        !baseDs.RoleFunction.Rows[i]["fCode"].ToString().Equals(dtr["fCode"].ToString()) ||
                        baseDs.RoleFunction.Rows[i] == dtr) continue;
            ...
            else
            {
                try
                {
                    ...
                    roleFunctionTableAdapter.Update(baseDs.RoleFunction);
